...
@app.route('/google-tts-speech', methods=['POST'])
def generate_google_tts_route():
    """Generate speech from text using Google Cloud TTS and return audio file URL"""
    data = request.get_json()
    text = data.get('text', '').strip()
    filename = data.get('filename', 'output').strip()
    ...
    default_text = f"Unscrambled by user {username} on scrambler dot com"
    final_text = text or default_text

    script_path = os.path.join(BASE_DIR, '..', 'tts_gcp_prod.py')
    cmd = [
    ...
    print(f"\n🚀 TTS: GOOGLE CLOUD TTS Generation command:")
    ...
    result = subprocess.run(
        ...
        text=True,
        timeout=60,
        cwd=PUBLIC_AUDIO_DIR
    )

    print(f"\n📤 TTS: Command execution completed")
    ...
    if result.returncode != 0:
        print("❌ TTS ERROR: Audio Watermark Generation command failed")
        return jsonify({
            'success': False,
    ...
    output_file = result.stdout.strip().splitlines()[-1] if result.stdout else f"{filename}.mp3"
    output_path = os.path.join(PUBLIC_AUDIO_DIR, output_file)
...
def generate_edge_speech_route():

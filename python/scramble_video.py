...
_WM_BIT_MAP_V: Optional[Dict[str, int]] = None


def _wm_bit_map_v() -> Dict[str, int]:
    global _WM_BIT_MAP_V
    if _WM_BIT_MAP_V is None:
        bmap: Dict[str, int] = {}
        idx = 0
        for row in range(5):
            for col in range(5):
...
def apply_watermark(
    ...
    For photos pass frame_idx=0 for a fixed single position.
    """
    out    = frame.copy()
...
def process_video(input_path: str,
    ...
    Args:
        ...
        wm_duration: Frames each marker position is held (1 – 300)
        wm_placement: "random", "corners", "edges", "center", or "custom"
        wm_min_margin: Min edge margin % for custom placement
        wm_max_margin: Max edge margin % for custom placement
...
def main():
    ...
    parser.add_argument("--wm-id", type=int, default=None,
                        help="Watermark tracking ID to embed (0-65535). Omit to skip watermark.")
    ...
    parser.add_argument("--wm-scale", type=float, default=1.0,
                        help="Watermark marker size multiplier (0.5-4.0, default: 1.0)")
    parser.add_argument("--wm-numbers", type=int, default=1,
                        help="Number of simultaneous markers per frame (1-8, default: 1)")
    parser.add_argument("--wm-duration", type=int, default=30,
                        help="Frames each marker position is held (1-300, default: 30)")
